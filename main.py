...
class LoginScreen(DarkBaseScreen):
    def __init__(self, **kwargs):
        ...
        layout = MDBoxLayout(orientation="vertical", spacing=25, padding=[30, 50, 30, 30])

        layout.add_widget(MDLabel(
            text="[b]HARMAN TOWN HOUSING SCHEME[/b]",
            halign="center", markup=True, font_style="H5",
            theme_text_color="Custom", text_color=(1, 0.65, 0.0, 1)
        ))
        ...
        layout.add_widget(MDRaisedButton(text="SIGN IN", md_bg_color=(0.0, 0.7, 0.4, 1), on_release=self.login, size_hint=(1, None), height=48))
        layout.add_widget(MDRaisedButton(text="CREATE NEW ACCOUNT", md_bg_color=(1, 0.55, 0, 1), on_release=self.goto_signup, size_hint=(1, None), height=48))
    ...
    def login(self, *args):
        try:
            ...
            MDDialog(title="Error", text=f"Login error: {e}").open()
...
class SignupScreen(DarkBaseScreen):
    def __init__(self, **kwargs):
        ...
        layout.add_widget(MDLabel(
            ...
            halign="center", markup=True, font_style="H5",
            theme_text_color="Custom", text_color=(0.25, 0.65, 1, 1)
        ))
        ...
        layout.add_widget(MDRaisedButton(text="SIGN UP", md_bg_color=(0.1, 0.85, 0.5, 1), on_release=self.register, size_hint=(1, None), height=48))
        layout.add_widget(MDRaisedButton(text="BACK TO LOGIN", md_bg_color=(0.45, 0.45, 0.45, 1), on_release=self.back, size_hint=(1, None), height=48))
        self.add_widget(layout)

    def register(self, *args):
        try:
            if create_user(self.new_user.text.strip(), self.new_pass.text.strip()):
                MDDialog(title="Success", text="Account created successfully! You can now login.").open()
                self.manager.current = "login"
            else:
                MDDialog(title="Error", text="Username already exists").open()
        except Exception as e:
            MDDialog(title="Error", text=f"Signup error: {e}").open()
...
class SimulateScreen(DarkBaseScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        layout = MDBoxLayout(orientation="vertical", spacing=30, padding=[30, 50, 30, 30])

        layout.add_widget(MDLabel(
            text="[b]SCAN YOUR RECEIPT[/b]",
            halign="center", markup=True, font_style="H5",
            theme_text_color="Custom", text_color=(1, 0.55, 0, 1)
        ))

        scan_btn = MDRaisedButton(text="START QR SCANNER", md_bg_color=(1, 0.55, 0, 1), on_release=self.goto_camera, size_hint=(1, None), height=52)
...
class CameraScreen(DarkBaseScreen):
    def __init__(self, **kwargs):
        ...
        self.info_label = MDLabel(
            ...
            theme_text_color="Custom", text_color=(1, 0.6, 0.15, 1)
        )
        self.back_btn = MDRaisedButton(text="BACK", on_release=self.back_to_simulate, md_bg_color=(0.7, 0.7, 0.7, 1), size_hint=(1, None), height=44)
    ...
    def on_enter(self):
        try:
            ...
            if not self.capture.isOpened():
                ...
                return
            self.capture_event = Clock.schedule_interval(self.update, 1.0 / 30.0)
        except Exception as e:
    ...
    def on_leave(self):
        try:
            ...
            if self.capture_event:
                Clock.unschedule(self.capture_event)
        except:
            pass
    ...
    def update(self, dt):
        ...
        try:
            ...
            for obj in decoded_objs:
                ...
                if "client_login://" in qr_data and "pin=" in qr_data:
                    Clock.unschedule(self.capture_event)
                    self.capture.release()
    ...
    def process_qr(self, qr_data):
        try:
            ...
            if client:
                ...
                self.info_label.text = "Client not found or invalid PIN."
        except Exception as e:
            self.info_label.text = f"QR Processing Error: {e}"
...
class ResultScreen(DarkBaseScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.client_data = {}
        layout = MDBoxLayout(orientation="vertical", spacing=20, padding=[30, 40, 30, 30])
        ...
        self.back_button = MDRaisedButton(
            text="SCAN AGAIN", on_release=self.back_to_simulate, md_bg_color=(0.4, 0.75, 0.25, 1), size_hint=(1, None), height=48
        )
    ...
    def on_pre_enter(self, *args):
        try:
            ...
            self.info_label.text = f"Data Error: {e}"
